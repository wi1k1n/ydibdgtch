...
void CGSGraph::AddNode(CGSNode& node) {
	if (!node.GetParent()) {
		LOGLN("CGSGraph::AddNode called with node that has no parent!");
		return;
	}
...
void CGSGraph::GetNodesAtLevel(uint16_t level, Array<CGSNode>& outNodes) {
	...
	//outNodes.clear();
	//int cursorIdx = 0;
	//while (cursorIdx < nodes.size()) {
	//	const CGSNode& node = nodes[cursorIdx];
	//	uint16_t nodeLevel = levels[cursorIdx];
